...
open_nfc_custom_write(struct file * filp, const char __user * buf, size_t count, loff_t * f_pos)
{
	struct open_nfc_custom_device *p_device = filp->private_data;
	int             retval;
	uint8_t         tmpbuf[I2C_READ_DATA_LENGTH];
	size_t          sent = 0;
	...
	while (sent < count) {
		/* bytes to send: min( remaining, I2C_READ_DATA_LENGTH ) */
		size_t          tosend = I2C_READ_DATA_LENGTH;
		if ((count - sent) < I2C_READ_DATA_LENGTH)
			tosend = count - sent;
...
open_nfc_custom_init(void)
{
	...
	if (retval < 0) {
		...
		printk("failed to assign IRQ! error = %d\n", retval);
		goto end;
	}
