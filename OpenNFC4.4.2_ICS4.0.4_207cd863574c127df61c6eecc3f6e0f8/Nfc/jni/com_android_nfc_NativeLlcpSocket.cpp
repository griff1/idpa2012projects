...
static jboolean com_android_nfc_NativeLlcpSocket_doConnectBy(JNIEnv *e, jobject o, jstring sn)
{
	...
	LogInformation("com_android_nfc_NativeLlcpSocket_doConnectBy: isCopy=%s length=%d, sn=%s",
			((isCopy==JNI_TRUE) ? "TRUE" : "FALSE"), serviceNameLength, sn);
	...
	if(socket->status == SOCKET_ERROR)
	{
		LogError("Can't connect to the p2p device (service  name = %s)", sn);
		status = JNI_FALSE;
	}
