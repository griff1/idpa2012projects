...
#define P_7816_4_INS_GETREPONSE      0xC0

/** The minimum length (in bytes) of an AID */
#define P_SEHALSM_MIN_AID_LENGTH 5
/** The maximum length (in bytes) of an AID */
#define P_SEHALSM_MAX_AID_LENGTH 16
...
typedef struct __tPSeHalSmChannel
{
   ...
   uint8_t aAID[P_SEHALSM_MAX_AID_LENGTH];
                  ...
                  uint8_t nLe)
{
   ...
   pInstance->current.aReaderToCard[0] = P_7816_4_CLA; /* CLA byte is set later */
   pInstance->current.aReaderToCard[1] = P_7816_4_INS_GETREPONSE;
                  ...
                  tPSeHalSmChannel* pChannel)
{
   ...
   pInstance->pRawInterface->pCloseChannel(pContext,
      pInstance->pRawInstance,
      pChannel->nChannelIdentifier,
                  ...
                  W_ERROR nResult )
{
   ...
   if (nResult == W_SUCCESS) for(;(pInstance->current.nState == P_SEHALSM_STATE_EXCHANGE_PENDING) || (pInstance->current.nChannelType != W_7816_CHANNEL_TYPE_RAW);)
   {
      ...
      if (pCardToReader[nDataLength - 2] == P_7816_4_SW1_61)
      {
      ...
      if (pCardToReader[nDataLength - 2] == P_7816_4_SW1_6C)
      {
   ...
   switch(pInstance->current.nState)
   {
      ...
      case P_SEHALSM_STATE_EXCHANGE_PENDING:
      {
         ...
         else if (nResult == W_ERROR_BUFFER_TOO_SHORT)
         {
            ...
            pInstance->pRawInterface->pGetResponseApdu(pContext,
               pInstance->pRawInstance,
               pChannel->nChannelIdentifier,
               (uint8_t*)null, 0,
               &nActualLength);
            ...
            {
               /* The pGetResponseApdu function did nothing ?!? */
               nActualLength = 0;
                  ...
                  tPSeHalSmInstance** ppInstance)
{
   ...
   {
      PDebugError("PSeHalSmCreateInstance: pRawInterface == null");
      return W_ERROR_BAD_PARAMETER;
   }

   if (pRawInterface->pExchange == null)
   {
      PDebugError("PSeHalSmCreateInstance: pRawInterface->pExchange == null");
      return W_ERROR_BAD_PARAMETER;
   }

   if (pRawInstance == null)
   {
      PDebugError("PSeHalSmCreateInstance: pRawInstance == null");
      return W_ERROR_BAD_PARAMETER;
   }
...
/* See the definition of tPSeHalSmOpenLogicalChannel in header file */
   ...
   switch(nType)
   {
      ...
      case W_7816_CHANNEL_TYPE_LOGICAL:
      {
         ...
         if (pAID != null)
         {
            if (nAIDLength == 2) /* MF */
            {
               if ((pAID[0] != 0x3F) || (pAID[1] != 0x00))
               {
                  PDebugError("static_PSeHalSmOpenChannel: pAID != null && nAIDLength == 2 but pAID does not contain 0x3F, 0x00");
                  return W_ERROR_BAD_PARAMETER;
               }
            }
            else if ((nAIDLength < P_SEHALSM_MIN_AID_LENGTH) || (nAIDLength > P_SEHALSM_MAX_AID_LENGTH))
            {
...
open_channel:
   ...
   pInstance->pRawInterface->pOpenChannel(pContext,
      pInstance->pRawInstance,
...
/* See the definition of tPSeHalSmCloseChannel in header file */
...
/* See the definition of tPSeHalSmExchangeApdu in header file */
                  ...
                  uint32_t nReceivedApduBufferMaxLength)
{
   ...
   if (pChannel != &pInstance->sRawChannel)
   {
      ...
      if ((nSendApduBufferLength >= 2) && (pSendApduBuffer[1] == P_7816_4_INS_MANAGE_CHANNEL))
      {
      ...
      /* Check SELECT [AID] */
      if ((nSendApduBufferLength >= 3) && (pSendApduBuffer[1] == P_7816_4_INS_SELECT) && (pSendApduBuffer[2]== P_7816_4_INS_SELECT_AID_P1))
      {
...
static tSESlot* static_PSeHalSmLocateSlot(tSEInstance* pSEInstance, uint32_t nHalSlotIdentifier)
{
   uint32_t nIndex;
   for(nIndex = 0; nIndex < pSEInstance->nSeNumber; nIndex++)
   {
      if (pSEInstance->pSlotArray[nIndex].nHalSlotIdentifier == nHalSlotIdentifier)
      {
         return &pSEInstance->pSlotArray[nIndex];
      }
   }
                  ...
                  uint32_t nParam2)
{
   ...
   tSESlot* pSlot = static_PSeHalSmLocateSlot(pSEInstance, nHalSlotIdentifier);
   tPSeHalSmInstance* pSeHalSmInstance;

   if (pSlot == null)
   {
      PDebugError("PSeHalSmNotifyOperationCompletion: Invalid nSlotIdentifier parameter");
      return;
   }
   ...
   switch(nOperation)
   {
      ...
      case C_SE_OPERATION_OPEN:
      {
         ...
         if (pSeHalSmInstance->current.pChannel != null)
         {
            ...
            pSeHalSmInstance->pRawInterface->pGetResponseApdu(pContext,
               pSeHalSmInstance->pRawInstance,
