...
const uint8_t PSecurityStackPkcs15Aid[] = { 0xA0, 0x00, 0x00, 0x00, 0x63, 0x50, 0x4B, 0x43, 0x53, 0x2D, 0x31, 0x35 };

/** @brief Length of the AID of the PKCS#15 application */
const uint32_t PSecurityStackPkcs15AidLength = sizeof(PSecurityStackPkcs15Aid);

/** @brief AID of the MF */
const uint8_t PSecurityStackMfAid[] = { 0x3F, 0x00 };

/** @brief Length of the AID of the MF */
const uint32_t PSecurityStackMfAidLength = sizeof(PSecurityStackMfAid);
...
#define P_SECSTACK_INS_READ_RECORD ((uint8_t)0xB2)
...
)
{
   ...
   aReaderToCardBuffer[3] = P_SECSTACK_P2_SELECT_FILE;
...
)
{
   ...
   aReaderToCardBuffer[0] = P_SECSTACK_CLA_ISO7816;
   aReaderToCardBuffer[1] = P_SECSTACK_INS_READ_RECORD;
                  ...
                  uint32_t* pnActualDfPkcs15PathCount)
{
   ...
   do
   {
      if (PAsn1GetTagValue(&sIncludedParser) == 0x4F)
      {
         ...
         bAidFound = ((nAidLength == PSecurityStackPkcs15AidLength)
            && (CMemoryCompare(PSecurityStackPkcs15Aid, pAidBuffer, nAidLength) == 0));
   ...
   switch(pSecurityStackInstance->sReadAcl.nState)
   {
      case P_STATE_OPEN_CHANNEL_SELECT_PKCS15_APPLICATION:
      {
         ...
         static_PSecurityStackLoadAclStateMachineOpenLogicalChannel(
            pContext,
            pSecurityStackInstance,
            PSecurityStackPkcs15Aid, PSecurityStackPkcs15AidLength);
      ...
      case P_STATE_SELECT_MF:
      {
         ...
         static_PSecurityStackLoadAclStateMachineOpenLogicalChannel(
            pContext,
            pSecurityStackInstance,
            PSecurityStackMfAid, PSecurityStackMfAidLength);
