...
)
{
   ...
   pAclProcessorInstance->sReadAcf.aReaderToCardBuffer[0] = P_SECSTACK_CLA_ISO7816;
   pAclProcessorInstance->sReadAcf.aReaderToCardBuffer[1] = P_SECSTACK_INS_READ_BINARY;
                  ...
                  uint32_t nApduLength)
{
   ...
   if (pApdu[1] != (uint8_t)0xD6) /* UPDATE BINARY */
   {
      /* This is not an UPDATE BINARY command */
      return W_SUCCESS;
   }

   if ((nAidLength != PSecurityStackPkcs15AidLength) || (CMemoryCompare(PSecurityStackPkcs15Aid, pAid, nAidLength) != 0))
   {
