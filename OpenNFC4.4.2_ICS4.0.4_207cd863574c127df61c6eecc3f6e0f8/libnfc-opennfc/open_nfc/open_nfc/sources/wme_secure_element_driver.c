...
#define P_SE_CLA_ISO7816 ((uint8_t)0x00)

/** The INS byte of the SELECT command */
#define P_SE_INS_SELECT ((uint8_t)0xA4)

/** The P1 byte of the SELECT[AID] command */
#define P_SE_P1_SELECT_AID ((uint8_t)0x04)

/** The P2 byte of the SELECT[AID] command */
#define P_SE_P2_SELECT_AID ((uint8_t)0x00)

/** The INS byte of the MANAGE CHANNEL command */
#define P_SE_INS_MANAGE_CHANNEL ((uint8_t)0x70)

/** The P1 byte of the MANAGE CHANNEL[open] command */
#define P_SE_P1_MANAGE_CHANNEL_OPEN ((uint8_t)0x00)

/** The P1 byte of the MANAGE CHANNEL[close] command */
#define P_SE_P1_MANAGE_CHANNEL_CLOSE ((uint8_t)0x80)

/** The minimum length (in bytes) of an AID */
#define P_SE_MIN_AID_LENGTH 5
/** The maximum length (in bytes) of an AID */
#define P_SE_MAX_AID_LENGTH 16
...
static void static_PSmRawExchangeApduCompletion(
            tContext* pContext,
            void* pCallbackParameter,
            ...
            W_ERROR nError )
{
   ...
   if(nError != W_SUCCESS)
   {
      PDebugError("static_PSmRawExchangeApduCompletion: Failed with error %s", PUtilTraceError(nError));
   }
...
static void static_P14P4SmRawExchangeApdu(
                  tContext* pContext,
                  ...
                  uint32_t nReceivedApduBufferMaxLength)
{
   PDFCFillCallbackContext(
      ...
      &pInstance->pSlot->sExchange.sCallbackContext );

   CDebugAssert(pInstance->pSlot->hDriverConnection != W_NULL_HANDLE);

   (void)P14P4DriverExchangeDataInternal(
      pContext,
      pInstance->pSlot->hDriverConnection,
      static_PSmRawExchangeApduCompletion,
      pInstance->pSlot,
...
static tP7816SmRawInterface static_P14P4SmRawInterface =
{
   /** The function that is called to perform an APDU exchange */
   static_P14P4SmRawExchangeApdu
};
            ...
            tSEInstance* pSEInstance)
{
   ...
   if(pSEInstance->bSEInfoIsBuilt == W_FALSE)
   {
      ...
      for(nSlotIdentifier = 0; nSlotIdentifier < nSeNumber; nSlotIdentifier++)
      {
         ...
         tP7816SmRawInstance* pSeSmRawInstance;
         ...
         /* @todo - This pSeSmRawInstance may actually be replaced by pSlot */
         pSeSmRawInstance = (tP7816SmRawInstance*)CMemoryAlloc(sizeof(*pSeSmRawInstance));
         if (pSeSmRawInstance == null)
         {
            PDebugError("static_PSEBuildInfo: Failed to allocate tP7816SmRawInstance");
            nError = W_ERROR_OUT_OF_RESOURCE;
            goto return_function;
         }
         CMemoryFill(pSeSmRawInstance, 0, sizeof(*pSeSmRawInstance));
         pSeSmRawInstance->pSlot = pSlot;
         ...
         if((pSlot->sConstantInfo.nCapabilities & W_SE_FLAG_COMMUNICATION_VIA_RF) != 0)
         {
            ...
            nError = P7816SmCreateInstance(pContext, &static_P14P4SmRawInterface,
               pSeSmRawInstance, &pSlot->s7816Sm.pLowLevelSmInstance);
         }
#ifdef P_INCLUDE_SE_SECURITY
         else if((pSlot->sConstantInfo.nCapabilities & W_SE_FLAG_COMMUNICATION) != 0)
         {
            ...
            nError = PSeHalSmCreateInstance(pContext, &g_PSeHalSmRawInterface,
               (tPSeHalSmRawInstance*)pSeSmRawInstance,
         ...
         if (nError != W_SUCCESS)
         {
            ...
            CMemoryFree(pSeSmRawInstance);
            pSeSmRawInstance = (tP7816SmRawInstance*)null;

            nError = W_ERROR_OUT_OF_RESOURCE;
            goto return_function;
         }

         pSlot->pSeSmRawInstance = pSeSmRawInstance;
            ...
            tContext* pContext )
{
   ...
   if(pSEInstance->bSEInfoIsBuilt != W_FALSE)
   {
      ...
      for(nSlotIdentifier = 0; nSlotIdentifier < pSEInstance->nSeNumber; nSlotIdentifier++)
      {
         ...
         /* @todo - This pSeSmRawInstance may actually be replaced by pSlot - In this case, do not free it! */
         if (pSlot->pSeSmRawInstance != null)
         {
            CMemoryFree(pSlot->pSeSmRawInstance);
            pSlot->pSeSmRawInstance = (tP7816SmRawInstance*)null;
         }
      }
         ...
         W_ERROR nError)
{
   ...
   if(nError != W_SUCCESS)
   {
      PDebugError("static_PSEMonitorEndOfTransactionProcessAIDListCompleted: receiving error %s, ignore",
         PUtilTraceError(nError));
      return;
   }
         ...
         uint32_t nNALMessageReceptionCounter)
{
   ...
   {
      /* NB: ACL will be reloaded when the SE connection is opened before reading the transaction hitory */
      ...
      if(nError == W_ERROR_RETRY)
      {
         /* Asynchronous operation pending, just return */
         return;
      }
   }
...
return_function:
   ...
   /* Filter the AID and send the event */
#ifdef P_INCLUDE_SE_SECURITY
   if ((pSlot->sMonitorEndOfTransaction.nLastAidListLength > P_SE_MIN_AID_LENGTH) && (pSlot->pSecurityStackInstance != null))
   {
...
/* See header file */
W_ERROR PSEDriverSetTransactionParameter(
                  tContext* pContext,
                  uint32_t nSlotIdentifier,
                  const uint8_t* pAid,
                  uint32_t nAidLength,
                  ...
                  uint32_t nParameterLength)
{
   ...
   if (PNFCControllerIsActive(pContext) == W_FALSE)
   {
      PDebugError("PSESetTransactionParameter: bad NFC Controller mode");
   ...
   if(nSlotIdentifier >= pSEInstance->nSeNumber)
   {
      PDebugError("PSESetTransactionParameter: Wrong SE slot identifier");
      return W_ERROR_BAD_PARAMETER;
   }
   pSlot = &pSEInstance->pSlotArray[nSlotIdentifier];

   if((nAidLength > P_SE_MAX_AID_LIST_LENGTH) || (nParameterLength > P_SE_MAX_CONNECTIVITY_DATA_LENGTH))
   {
      PDebugError("PSESetTransactionParameter: Buffer too short");
      return W_ERROR_BUFFER_TOO_SHORT;
   }

   pSlot->sMonitorEndOfTransaction.nLastAidListLength = nAidLength;

   if(nAidLength != 0)
   {
      CMemoryCopy(pSlot->sMonitorEndOfTransaction.aLastAidList, pAid, nAidLength);
   }

   pSlot->sConnectivity.nDataLength = nParameterLength;

   if(nParameterLength != 0)
   {
      CMemoryCopy(pSlot->sConnectivity.aDataBuffer, pParameterBuffer, nParameterLength);
   }

   return W_SUCCESS;
}

/* Returnes the slot with the specified HAL identifier, null if not found */
static tSESlot* static_PSEGetSlotFromHalIdentifier(
            tContext* pContext,
            ...
            bool_t bIsPresent)
{
   ...
   pSlot = static_PSEGetSlotFromHalIdentifier(pContext, nHalSlotIdentifier);
...
static void static_PSEApplyPolicyCompleted(
         tContext* pContext,
         void* pCallbackParameter,
         W_ERROR nError)
{
   ...
   PDebugTrace("static_PSEApplyPolicyCompleted()");
         ...
         W_ERROR nError)
{
   ...
   if((nError != W_SUCCESS)
   || ((pSlot->nPendingFlags & P_SE_FLAG_PENDING_JUPITER) == 0))
   {
      pSlot->nPendingFlags &= ~P_SE_FLAG_PENDING_SET_POLICY;
   ...
   PJupiterApplyPolicy(
               pContext,
               pSlot->sConstantInfo.nSlotIdentifier,
               static_PSEApplyPolicyCompleted, pSlot);
}
            ...
            void* pCallbackParameter)
{
   ...
   if((pSEInstance->pSlotArray[nSlotIdentifier].nConstantProtocols & nProtocols) != nProtocols)
   {
      PDebugError("Bad SE policy parameter");
      nError = W_ERROR_FEATURE_NOT_SUPPORTED;
      goto send_event;
   ...
   {
      PDebugError("A standalone SE has no policy to be set");
      nError = W_ERROR_FEATURE_NOT_SUPPORTED;
      goto send_event;
   }
   ...
   for(nIndex = 0; nIndex < pSEInstance->nSeNumber; nIndex++)
   {
      ...
      {
         PDebugError("PSESetPolicy: internal communication already active");
         nError = W_ERROR_BAD_STATE;
         goto send_event;
      }
   }
   ...
   W_ERROR nError)
{
   ...
   if(nError != W_SUCCESS)
   {
      PDebugError("static_PSEOperationCloseChannelCompleted: returning error %s",
         PUtilTraceError(nError));
      if(pSlot->sOperation.nCurrentError == W_SUCCESS)
      {
         pSlot->sOperation.nCurrentError = nError;
            ...
            uint32_t nImpersonationDataBufferLength )
{
   ...
   if((pAIDBuffer == null) || (nAIDLength < P_SE_MIN_AID_LENGTH) ||  (nAIDLength > P_SE_MAX_AID_LENGTH)
