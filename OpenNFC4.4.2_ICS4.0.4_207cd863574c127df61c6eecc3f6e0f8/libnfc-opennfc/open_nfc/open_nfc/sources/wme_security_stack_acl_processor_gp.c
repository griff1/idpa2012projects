...
/**
 * @brief Parses the answer (FCI) to the SELECT command returned by the GlobalPlatform-compliant DF or application.
 *
 * @param[in]  pAclProcessorInstance  The ACL Processor instance.
 *
 * @param[in]  pFcpBuffer  The FCP buffer.
 ...
 *          - W_ERROR_BAD_PARAMETER  The FCI is malformed.
 */
static W_ERROR static_PSecurityStackGpAclProcessorParseAnswerToSelect(
                  tPSecurityStackAclProcessorInstance* pAclProcessorInstance,
                  const uint8_t* pFcpBuffer,
                  uint32_t nFcpLength)
{
   ...
   PDebugTrace("static_PSecurityStackGpAclProcessorParseAnswerToSelect()");

   /*
    * The answer to the SELECT[AID] command is (likely) formatted as:
    * 62 xx                FCP template
    *    82 02 78 21       File descriptor: DF/Linear variable, Data Coding:Write Or/2 nibbles
    *    xx ...
    */

   nResult = static_PSecurityStackGpAclProcessorExtractFieldFromFcp(pAclProcessorInstance,
     pFcpBuffer, nFcpLength, 0x82, &pFileDescriptorBuffer, &nFileDescriptorLength);
   if (nResult != W_SUCCESS)
   {
      return nResult;
   }

   if (nFileDescriptorLength < 2)
   {
      PDebugTrace("static_PSecurityStackGpAclProcessorParseAnswerToSelect: Malformed tag '82'");
      return W_ERROR_BAD_PARAMETER;
   }

   if ((pFileDescriptorBuffer[0] != 0x78) && (pFileDescriptorBuffer[1] != 0x21))
   {
      PDebugTrace("static_PSecurityStackGpAclProcessorParseAnswerToSelect: Incorrect tag '82'");
      return W_ERROR_BAD_PARAMETER;
   }
                  ...
                  tPSecurityStackAclProcessorInstance** ppAclProcessorInstance)
{
   ...
   if (nAnswerToSelectLength < 2)
   {
      PDebugError("static_PSecurityStackGpAclProcessorCreateInstance: Buffer is too short (%d), must be at least 2", nAnswerToSelectLength);
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }

/*
 * Allow FCP (tag=6F) even if selection by AID
 *
   if ((bIsSelectedByAid != W_FALSE) && (nAnswerToSelectLength > 2) && (pAnswerToSelect[0] != 0x6F))
   {
      PDebugTrace("static_PSecurityStackGpAclProcessorCreateInstance: Must be selected by FID");
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
*/
   ...
   nAnswerToSelectLength -= 2; /* Remove SW */

   /* Check whether the Answer-to-Select is recognized by this ACL processor */
   if (static_PSecurityStackGpAclProcessorParseAnswerToSelect(pAclProcessorInstance, pAnswerToSelect, nAnswerToSelectLength) != W_SUCCESS)
   {
      CMemoryFree(pAclProcessorInstance);
      pAclProcessorInstance = (tPSecurityStackAclProcessorInstance*)null;
      PDebugError("static_PSecurityStackGpAclProcessorCreateInstance: error returned by static_PSecurityStackGpAclProcessorParseAnswerToSelect()");
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
                  ...
                  bool_t bIsSelectedByAid)
{
   ...
   if (nAnswerToSelectLength < 2)
   {
      PDebugError("static_PSecurityStackGpAclProcessorUpdateInstance: Buffer is too short (%d), must be at least 2", nAnswerToSelectLength);
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
 ...
 *
   if ((bIsSelectedByAid != W_FALSE) && (nAnswerToSelectLength > 2) && (pAnswerToSelect[0] != 0x6F))
   {
      PDebugError("static_PSecurityStackGpAclProcessorUpdateInstance: Must be selected by FID");
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
*/
   ...
   if (static_PSecurityStackGpAclProcessorParseAnswerToSelect(pAclProcessorInstance, pAnswerToSelect, nAnswerToSelectLength) != W_SUCCESS)
   {
      PDebugError("static_PSecurityStackGpAclProcessorUpdateInstance: error returned by static_PSecurityStackGpAclProcessorParseAnswerToSelect()");
      return W_ERROR_CONNECTION_COMPATIBILITY;
   }
...
#define P_SECSTACK_P2_SELECT_FILE ((uint8_t)0x04)
...
)
{
   ...
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[0] = P_SECSTACK_CLA_ISO7816;
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[1] = P_SECSTACK_INS_SELECT;
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[2] = 0x00;
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[3] = P_SECSTACK_P2_SELECT_FILE;
...
)
{
   ...
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[0] = P_SECSTACK_CLA_ISO7816;
   pAclProcessorInstance->sReadAcl.aReaderToCardBuffer[1] = P_SECSTACK_INS_READ_BINARY;
...
)
{
   ...
   PDebugWarning("static_PSecurityStackGpAclProcessorParsePrincipals()");
                  ...
                  uint32_t nRefreshFileListLength)
{
   ...
   while(nNumberOfFiles-->0)
   {
      ...
      /* Path must start with 0x3F */
      if (pRefreshFileList[0] != 0x3F)
      {
         PDebugError("static_PSecurityStackGpAclProcessorNotifyStkRefresh: The Files path is inconsistent with the number of files");
         return W_ERROR_BAD_PARAMETER;
      }
      ...
      /* There must be a DF or an EF (must not start with 0x3F) */
      if (pRefreshFileList[0] == 0x3F)
      {
         PDebugError("static_PSecurityStackGpAclProcessorNotifyStkRefresh: The Files path does not contain a DF/EF after the MF");
         return W_ERROR_BAD_PARAMETER;
