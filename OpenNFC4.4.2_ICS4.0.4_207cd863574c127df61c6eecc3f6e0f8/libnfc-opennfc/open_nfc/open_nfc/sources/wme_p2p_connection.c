   ...
   W_ERROR        nError)
{
   ...
   {
      pP2PSocket->sConnection.bIsConnectCalled = W_TRUE;
...
)
{
   ...
   if(pP2PSocket->sConnection.bIsConnectCalled == W_TRUE)
   {
      PDebugError("PP2PConnect : connect already done");
      nError = W_ERROR_BAD_STATE;
      goto error;
   }
...
)
{
   ...
   else
   {
      ...
      PDFCFlushCall(&sCallbackContext);
   }
   return;
