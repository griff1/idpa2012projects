               ...
               uint16_t nFileID)
{
...

   {
      ...
      if (pNDEFConnection->sType.t4.nVariant == P_NDEF_4_VERSION_20)
      {
         pNDEFConnection->pSendBuffer[3] = 0x0C;
      }
...
static void static_PNDEFType4WriteFile(
               ...
               uint32_t nLength)
{
   /* Paylaod to erase the NDEF Length */
   static const uint8_t aEraseNDEFLength[] = { 0x00, 0xD6, 0x00, 0x00, 0x02, 0x00, 0x00 };
...
{
   /* Paylaod to erase the NDEF Length */
   uint8_t aEraseNDEFLength[] = { 0x00, 0xD6, 0x00, 0x00, 0x02,0x00, 0x00 };
   ...
   aEraseNDEFLength[5] =  ((nLength >> 8) & 0x000000FF);
   aEraseNDEFLength[6] =  (nLength & 0x000000FF);

   pNDEFConnection->sType.t4.nBytesToProcess = 2;
   pNDEFConnection->sType.t4.nBytesProcessed = 0;
   pNDEFConnection->sType.t4.nBytesPending   = 2;
