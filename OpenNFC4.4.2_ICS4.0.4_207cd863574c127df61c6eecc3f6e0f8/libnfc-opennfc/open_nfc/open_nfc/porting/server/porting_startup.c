            ...
            uint32_t nPrincipalBufferLength)
{
   ...
   {
      if((nPrincipalBufferLength % P_PRINCIPAL_LENGTH) != 0)
      {
         return W_FALSE;
      }

      /* User data case */
      if(nAuthenticationDataLength != P_PRINCIPAL_LENGTH)
      {
         return W_FALSE;
      }

      while(nPrincipalBufferLength != 0)
      {
         ...
         pPrincipalBuffer += P_PRINCIPAL_LENGTH;
      }
            ...
            uint32_t nImpersonationDataBufferLength)
{
   ...
   /* User data case */
   if(nImpersonationDataBufferLength != P_PRINCIPAL_LENGTH)
   {
      return W_FALSE;
   }
